...
const handler = createMcpHandler((server) => {
    ...
    {
      id: z.number().describe("Unique numeric identifier for the pet").optional(),
      name: z.string().describe("The pet's name"),
      category: z.record(z.unknown()).describe("Category object containing pet classification information").optional(),
      photoUrls: z.array(z.unknown()).describe("Array of photo URLs for the pet"),
      tags: z.array(z.unknown()).describe("Array of tags to label the pet").optional(),
      status: z.string().describe("Current status of the pet in the store (available, pending, sold)").optional(),
    },
  ...
  server.tool(
    "update_pet",
    "Call this to update an existing pet's information. Provide the pet's ID and new details like name, photo URLs, category, tags, or status. Returns the updated pet object.",
    {
      id: z.number().describe("Unique numeric identifier for the pet to update").optional(),
      name: z.string().describe("The pet's name"),
      category: z.record(z.unknown()).describe("Category object containing pet classification information").optional(),
      photoUrls: z.array(z.unknown()).describe("Array of photo URLs for the pet"),
      tags: z.array(z.unknown()).describe("Array of tags to label the pet").optional(),
  ...
  server.tool(
    ...
    "Call this to find all pets matching one or more statuses. Provide comma-separated status values like 'available,pending' to filter the results. Returns an array of matching pets.",
    {
      status: z.string().describe("Comma-separated list of pet statuses to filter by (available, pending, sold)"),
    },
    async (params, { authInfo }) => {
      const creds = await fetchCredentials(authInfo?.token)
  ...
  server.tool(
    "search_pets_by_tags",
    "Call this to find all pets matching one or more tags. Provide comma-separated tag values like 'tag1,tag2' to filter the results. Returns an array of matching pets.",
    {
      tags: z.array(z.unknown()).describe("Comma-separated list of tags to filter pets by"),
    },
    async (params, { authInfo }) => {
      const creds = await fetchCredentials(authInfo?.token)
      const _params = new URLSearchParams()
  ...
  server.tool(
    "get_pet_by_id",
    "Call this to retrieve a single pet's details by its numeric ID. Returns the complete pet object including name, status, category, tags, and photo URLs.",
    {
  ...
  server.tool(
    ...
    "Call this to update a pet's name and/or status using form data. Provide the pet's ID and optionally its new name or status. Returns the updated pet.",
    {
      petId: z.number().describe("The numeric ID of the pet to update"),
      name: z.string().describe("New name for the pet").optional(),
      status: z.string().describe("New status for the pet (available, pending, sold)").optional(),
    },
    async (params, { authInfo }) => {
      const creds = await fetchCredentials(authInfo?.token)
      const url = `${BASE_URL}/pet/${encodeURIComponent(String(params.petId))}`
  ...
  server.tool(
    "delete_pet",
    "Call this to delete a pet from the store by its numeric ID. Returns a success message when the pet is deleted.",
    {
  ...
  server.tool(
    "upload_pet_image",
    "Call this to upload an image file for a pet. Provide the pet's ID and optionally additional metadata. Returns confirmation of the uploaded image.",
    {
  ...
  server.tool(
    "get_store_inventory",
    "Call this to retrieve a summary of pet inventory by status. Returns a count of pets in each status (available, pending, sold).",
    {
  ...
  server.tool(
    "create_order",
    "Call this to place a new order for a pet in the store. Provide the pet ID, quantity, and optionally order ID, ship date, status, and completion flag. Returns the created order.",
    {
      id: z.number().describe("Order identifier").optional(),
      petId: z.number().describe("ID of the pet being ordered").optional(),
      quantity: z.number().describe("Number of items being ordered").optional(),
      shipDate: z.string().describe("Expected ship date for the order").optional(),
      status: z.string().describe("Order status (pending, approved, delivered)").optional(),
  ...
  server.tool(
    "get_order_by_id",
    "Call this to retrieve an order by its numeric ID. Use IDs <= 5 or > 10 for valid results. Returns the complete order details including pet ID, quantity, and status.",
    {
  ...
  server.tool(
    "delete_order",
    "Call this to delete an order by its numeric ID. Use IDs less than 1000 for valid results. Returns a success message when the order is deleted.",
    {
  ...
  server.tool(
    ...
    "Call this to create multiple user accounts at once. Provide an array of user objects with usernames, passwords, and optional details. Returns confirmation of created users.",
    {
  ...
  server.tool(
    "login_user",
    "Call this to log a user into the system. Provide username and password in clear text. Returns a session token or authentication confirmation on successful login.",
    {
  ...
  server.tool(
    "logout_user",
    "Call this to log out the currently authenticated user and end their session. Returns a success message.",
    {
  ...
  server.tool(
    "update_user",
    "Call this to update an existing user's information. Provide the username and any new details like email, phone, first/last name, or password. Returns the updated user object.",
    {
      username: z.string().describe("The username of the user to update"),
      id: z.number().describe("Unique numeric identifier for the user").optional(),
      ...
      userStatus: z.number().describe("User status code").optional(),
    },
    async (params, { authInfo }) => {
      const creds = await fetchCredentials(authInfo?.token)
  ...
  server.tool(
    "delete_user",
    "Call this to delete a user account by username. Returns a success message when the user is deleted.",
    {
