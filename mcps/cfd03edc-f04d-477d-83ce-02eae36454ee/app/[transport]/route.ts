...
const handler = createMcpHandler((server) => {
  ...
  server.tool(
    "update_pet",
    "Call this to update an existing pet's information by its ID. Provide the pet's ID, name, photo URLs, and optionally its category, tags, and status. Returns the updated pet object.",
    {
  ...
  server.tool(
    ...
    {
      status: z.string().describe("One or more status values to filter by, such as available, pending, or sold (comma-separated)"),
    },
    async (params, { authInfo }) => {
      const creds = await fetchCredentials(authInfo?.token)
  ...
  server.tool(
    ...
    {
      tags: z.array(z.unknown()).describe("One or more tags to filter by (comma-separated, e.g., tag1, tag2, tag3)"),
    },
    async (params, { authInfo }) => {
      const creds = await fetchCredentials(authInfo?.token)
      const _params = new URLSearchParams()
  ...
  server.tool(
    "get_pet_by_id",
    "Call this to retrieve a single pet's full details by its numeric ID. Returns the complete pet object including name, status, tags, and photos.",
    {
  ...
  server.tool(
    ...
    "Call this to update a pet's name and/or status using form data. Provide the pet's ID and optionally its new name and status. Returns success confirmation.",
    {
  ...
  server.tool(
    "delete_pet",
    "Call this to delete a pet from the store by its ID. Returns success confirmation.",
    {
  ...
  server.tool(
    "upload_pet_image",
    "Call this to upload an image file for a pet. Provide the pet's ID and optionally additional metadata. Returns the upload response with file details.",
    {
  ...
  server.tool(
    "get_order_by_id",
    "Call this to retrieve a purchase order by its numeric ID. Returns the complete order details including pet ID, quantity, ship date, and status. Note: Use IDs <= 5 or > 10 for valid responses.",
    {
  ...
  server.tool(
    "delete_order",
    "Call this to delete a purchase order by its numeric ID. Returns success confirmation. Note: Use IDs < 1000 for valid responses.",
    {
  ...
  server.tool(
    ...
    "Call this to create multiple user accounts at once using a list of user objects. Returns success confirmation.",
    {
  ...
  server.tool(
    "logout_user",
    "Call this to log out the currently logged-in user and end their session. Returns success confirmation.",
    {
  ...
  server.tool(
    "update_user",
    "Call this to update an existing user's information by their username. Provide the username and optionally update their ID, first name, last name, email, password, phone, and status. Returns the updated user object.",
    {
  ...
  server.tool(
    "delete_user",
    "Call this to delete a user account by their username. Returns success confirmation.",
    {
